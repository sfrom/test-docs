...
namespace Acies.Docs.Services.Repositories
{
    public class DocumentRepository : IDocumentRepository
    {
        ...
        public DocumentRepository(IAmazonDynamoDB client, IDynamoDBContext dynamoDBContext, ISerializer serializer, DynamoDbDataRepositoryOptions options, TenantContext tenantContext)
        {
            _options = options;
        ...
        public async Task<Document?> GetAsync(string key)
        {
            var pk = "ACCOUNT#" + _tenantContext.AccountId.ToUpper();
            var sk = ("METADOC#" + key).ToUpper();
            DynamoDbDataItem r = await _dynamoDBContext.LoadAsync<DynamoDbDataItem>(pk, sk, _dbConfig);
            if (r == null) return default;
            return _serializer.Deserialize<Document>(r.Data);
        }

        public async Task<DocumentVersion?> GetVersionAsync(string key, int version)
        {
            var pk = ("DOCVERSION#" + JoinKeyVersion(key, version)).ToUpper();
            DynamoDbDataItem r = await _dynamoDBContext.LoadAsync<DynamoDbDataItem>(pk, pk, _dbConfig);
            if (r == null) return default;
            return _serializer.Deserialize<DocumentVersion>(r.Data);
        }

        public async Task<IEnumerable<Document>?> GetBatchAsync(IEnumerable<string> keys)
        {
            var batchget = _dynamoDBContext.CreateBatchGet<DynamoDbDataItem>(_dbConfig);
            foreach (var k in keys)
            {
                batchget.AddKey(("ACCOUNT#" + _tenantContext.AccountId).ToUpper(), ("METADOC#" + k));
            }
            ...
            {
                var doc = _serializer.Deserialize<Document>(r.Data);
                documents.Add(doc);
            }
            return documents;
        }

        public async Task<DocumentVersion?> GetLatestAsync(string key)
        {
            var pk = ("ACCOUNT#" + _tenantContext.AccountId + "#METADOC#" + key.ToUpper());
            ...
            var docVer = DocModel.Document.FromAttributeMap(r.Items[1]);
            var docVerData = _dynamoDBContext.FromDocument<DynamoDbDataItem>(docVer);
            return _serializer.Deserialize<DocumentVersion>(docVerData.Data);
        ...
        public async Task SaveAsync(Document data, string key, int version)
        {
        ...
        public async Task SaveVersionAsync(DocumentVersion data, string key, int version)
        {
            var pk = ("DOCVERSION#" + JoinKeyVersion(key, version)).ToUpper();
            var meta = ("METADOC#" + key).ToUpper();
